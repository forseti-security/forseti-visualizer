...
passport.use(new GoogleStrategy({
  clientID: _config["default"].oauth2ClientId,
  clientSecret: _config["default"].oauth2ClientSecret,
  callbackURL: _config["default"].oauth2Callback,
  accessType: 'offline'
...
var router = express.Router(); // [START middleware]
...
function addTemplateVariables(req, res, next) {
  ...
  res.locals.login = "/auth/login?return=".concat(encodeURIComponent(req.originalUrl));
  res.locals.logout = "/auth/logout?return=".concat(encodeURIComponent(req.originalUrl));
  next();
} // [END middleware]
...
'/auth/login', // Save the url of the user's current page so the app can redirect back to
...
router.get( // OAuth 2 callback url. Use this url to configure your OAuth client in the
// Google Developers console
'/auth/google/callback', // Finish OAuth 2 flow using Passport.js
passport.authenticate('google'), // Redirect back to the original page, if any
function (req, res) {
